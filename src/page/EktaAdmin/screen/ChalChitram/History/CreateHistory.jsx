...
import { createCourseAPI, createHistoryAPI, createPostAPI } from "../../../../../api";
...
export default function CreateHistory() {
  ...
  const [history_title, sethistory_title] = useState(false);
  ...
  const addPostFuncOnSubmit = async () => {
    ...
    const res = await createHistoryAPI(payload);
    if (res.data.success == true && res.status === 200)
      navigate(`/ektaAdmin/HistoryList`);
        ...
        <Grid container spacing={3}>
          <Grid item xs={12}>
            <div style={{ display: "flex" }}>
              ...
              >
                <Button variant="success" onClick={()=>navigate(`/EktaAdmin/HistoryList`)}>
                  <ArrowBackIcon />
                  Back
                </Button>
              </div>
          ...
          <Grid item xs={8}>
            <Form style={{ paddingLeft: "50px", paddingRight: "50px" }}>
              ...
              >
                {" "}
                <Form.Label style={{ color: "#4153a4" }}>History Titel</Form.Label>
                <Form.Control
                  style={{ backgroundColor: "#eeeff7" }}
                  type="text"
                  placeholder="name@example.com"
                  onChange={(e) => sethistory_title(e.target.value)}
                />
              </Form.Group>
